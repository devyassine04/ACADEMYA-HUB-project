...
class DepartementViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        print(f"DEBUG DEPARTEMENT: User={self.request.user}, Role={getattr(self.request.user, 'role', 'None')}, Count={queryset.count()}")
        
        # ADMIN and DIRECTION see everything
        if self.request.user.is_authenticated and self.request.user.role in ['ADMIN', 'DIRECTION']:
            return queryset
...
class InscriptionViewSet(viewsets.ModelViewSet):
    ...
    def pending(self, request):
        """
        ADMIN endpoint to get all pending inscriptions
