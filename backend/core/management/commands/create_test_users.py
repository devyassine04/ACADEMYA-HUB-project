...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options['reset']:
            ...
            self.stdout.write(
                self.style.WARNING(f'[TRASH] Deleted {deleted_count} existing test users')
            )
        ...
        # 2. Create Admin users (Chefs de département)
        ...
        # Summary
        if created_users:
            self.stdout.write(self.style.SUCCESS(f'\n[OK] Created {len(created_users)} test users:'))
            for user in created_users:
                self.stdout.write(f'   - {user}')
            self.stdout.write(self.style.SUCCESS('\n[KEY] All passwords: test123'))
        else:
            self.stdout.write(self.style.WARNING('[WARN] All test users already exist'))
